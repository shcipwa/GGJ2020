...
namespace FullscreenEditor {
    ...
    public static class FullscreenRects {
        ...
        public static Rect GetNonPrimaryBounds()
        {
            ...
            Vector2 minVals = Vector2.one * 99999999f;
            Vector2 maxVals = Vector2.one * -99999999f;

            try
            {
                for (var i = 0; i < displays.Count && i < 4; i++) {
                    if(displays[i].PrimaryDisplay && FullscreenPreferences.MosaicIgnoresPrimary)
                        continue;
 /*
                    var candidate = MenuItems.FindCandidateForFullscreen(Types.GameView, FullscreenUtility.GetMainGameView());
...
*/
                    var rect = displays[i].DpiCorrectedArea;
                    //fs.Rect = displays[i].DpiCorrectedArea;
                    //fs.Rect = Rect.MinMaxRect(fs.Rect.xMin,fs.Rect.yMin,fs.Rect.xMax,fs.Rect.yMax);

                    Debug.Log("Cycled window with rect: " + rect.ToString());
                    minVals = new Vector2(Mathf.Min(rect.xMin,minVals.x),
                        Mathf.Min(rect.yMin,minVals.y));
                    maxVals = new Vector2(Mathf.Max(rect.xMax,minVals.x),
                        Mathf.Max(rect.yMax,minVals.y));
                    //fs.Close();
                }
            ...
            var result = Rect.MinMaxRect(minVals.x, minVals.y, maxVals.x, maxVals.y);
